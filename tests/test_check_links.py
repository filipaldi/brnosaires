...
class Resolution(unittest.TestCase):
    ...
    def test_percent_encoded_reference_resolves_to_the_decoded_filename(self):
        self.assertEqual(self.build({
            "index.html": page('<img src="/images/a%20b.avif">'),
            "images/a b.avif": "binary",
        }), {})

    def test_escape_in_the_filename_itself_is_reported(self):
        # The file's name really contains `%28`, so the raw reference string
        # matches it on disk — but a browser decodes `%28` to `(` and asks for
        # a file that is not there. Matching the literal spelling made the gate
        # pass this for months; only the decoded one may count.
        broken = self.build({
            "index.html": page('<img src="/images/a%281%29.avif">'),
            "images/a%281%29.avif": "binary",
        })
        self.assertEqual(broken, {"/images/a%281%29.avif": ["index.html"]})

    def test_bare_percent_in_a_filename_still_resolves(self):
        # `%.a` is not a valid escape, so unquote leaves it alone.
        self.assertEqual(self.build({
            "index.html": page('<img src="/images/100%.avif">'),
            "images/100%.avif": "binary",
        }), {})
