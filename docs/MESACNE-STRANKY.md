...
Stačí normálně vytvořit soubor akce podle [Akce: přidat a upravit](AKCE.md), v [content/events/](../content/events/)`RRRR/MM/` s `event-type: milonga` (nebo `praktika`/`neolonga`) a `event-start` v daném měsíci — objeví se na příslušné měsíční stránce automaticky. Samotnou měsíční stránku neupravuješ.

## ⚠️ Co je v souboru a co (ne)měnit
...
- [Akce: přidat a upravit](AKCE.md) — jak vytvořit akci, která se na měsíční stránce objeví.
