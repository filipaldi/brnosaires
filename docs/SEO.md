...
Filtr `event_place(meta)` ([plugins/event_place.py](../plugins/event_place.py)) skládá z polí `event-venue` / `event-street` / `event-locality` `Place` (`name`) obalující `PostalAddress` (`streetAddress`/`addressLocality`/`addressCountry: "CZ"`), plus `line` — jeden lidský řetězec pro kartičku, ICS export a zrcadlo pro LLM. Prázdné pole se vynechává, ne zapisuje prázdné: `"streetAddress": ""` čte validátor jako tvrzení o adrese, ne jako mlčení. **Nikdy neemituje napůl postavenou `PostalAddress`.** Dřív to byl jeden řetězec `event-location` dělený podle čárek, takže tvar té věty nesl celou váhu a odchylka od něj tiše degradovala strukturovaná data; tři pole ten problém ruší. `"Brno"` v `addressLocality` je literální city signál pro `milonga Brno`.
